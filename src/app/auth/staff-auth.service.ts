...
export class StaffAuthService {
  ...
  async refreshSession(): Promise<void> {
    ...
    try {
      // Force refresh to pick up the latest cognito:groups claim (in case the user
      // was recently added to the Staff group in the Cognito console).
  ...
  /** Redirects the browser to the Cognito hosted UI for sign-in (OAuth code grant).
   *  Returns to /admin/login (or the configured redirectSignIn). The component
   *  will pick up the session on return via refreshSession().
   */
  async signInWithCognitoHosted(): Promise<void> {
