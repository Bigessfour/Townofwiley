...
class SiteMonitorTests(unittest.TestCase):
    def healthy_fetch_map(self) -> dict[tuple[str, str], object]:
        return {
            ...
            (
                "POST",
                "https://example.appsync-api.us-east-2.amazonaws.com/graphql",
            ): FakeResponse(
    ...
    def test_unhealthy_run_sends_email_with_failure_details(self) -> None:
        ...
        self.assertIn(
            "Town of Wiley site monitor detected a problem.", mailer.messages[0]["body"]
        )
        self.assertEqual(
            state_store.state["TownOfWileySiteMonitor"]["status"], "unhealthy"
        )
    ...
    def test_dynamo_state_store_uses_table_name(self) -> None:
        ...
        self.assertEqual(
            client.calls[0],
            {
                ...
                "Item": {
                    "monitorName": {"S": "TownOfWileySiteMonitor"},
                    "status": {"S": "healthy"},
                },
            },
        )
        ...
        self.assertEqual(
            loaded, {"monitorName": "TownOfWileySiteMonitor", "status": "healthy"}
        )
        self.assertEqual(
            client.calls[1],
            {
