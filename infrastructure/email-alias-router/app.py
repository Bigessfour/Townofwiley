...
class MailForwarder(Protocol):
    def forward(
        self, alias: EmailAliasRecord, raw_message: bytes, parsed_message: EmailMessage
    ) -> None: ...
...
def build_forward_email(
    ...
    forward_message["From"] = formataddr(
        (alias.display_name or "Town of Wiley Mail", forwarder_from)
    )
...
class SesMailForwarder:
    ...
    def forward(
        ...
        forward_message = build_forward_email(
            alias, raw_message, parsed_message, self._forwarder_from
        )
