...
class SevereWeatherBackendTests(unittest.TestCase):
    ...
    def test_returns_actionable_error_when_email_confirmation_delivery_fails(
        self,
    ) -> None:
        subscription_store = APP.MemorySubscriptionStore()
        ...
        self.assertIn(
            "Email confirmations are temporarily unavailable", response["body"]
        )
        self.assertIsNone(
            subscription_store.find_existing_subscription(
    ...
    def test_creates_spanish_subscription_and_translates_confirmation(self) -> None:
        ...
        self.assertEqual(response["statusCode"], 202)
        self.assertIn(
            "[es]", backend._notification_gateway.confirmation_messages[0]["message"]
        )
    ...
    def test_scheduled_event_translates_spanish_alert_messages_once(self) -> None:
        ...
        self.assertIn(
            "[es]", backend._notification_gateway.alert_messages[0]["subject"]
        )
        self.assertIn(
            "[es]", backend._notification_gateway.alert_messages[0]["message"]
        )
    ...
    def test_creates_pending_sms_subscription_and_sends_confirmation(self) -> None:
        ...
        self.assertEqual(
            gateway.confirmation_messages[0]["destination"], "+17195550101"
        )
        ...
        response = backend.handle(
            {
                ...
                "headers": {"host": "alerts.example.com", "x-forwarded-proto": "https"},
                "body": json.dumps(
                    {"channel": "sms", "destination": "123", "zipCode": "81092"}
                ),
            },
        )
        ...
        response = backend.handle(
            {
                ...
                "headers": {"host": "alerts.example.com", "x-forwarded-proto": "https"},
                "body": json.dumps(
                    {"channel": "push", "destination": "token123", "zipCode": "81092"}
                ),
            },
        )
    ...
    def test_duplicate_active_subscription_returns_200_with_unsubscribe_url(
        self,
    ) -> None:
        backend = build_backend()
        # Sign up and confirm
    ...
    def test_returns_actionable_error_when_sms_confirmation_delivery_fails(
        self,
    ) -> None:
        backend = APP.SevereWeatherBackend(
            config=APP.AppConfig(
    ...
    def test_developer_test_route_sends_only_explicit_sms_and_email_recipients(
        self,
    ) -> None:
        ...
        self.assertEqual(
            {message["channel"] for message in gateway.alert_messages}, {"email", "sms"}
        )
        self.assertIn(
            "steve.mckitrick@townofwiley.gov",
            {message["destination"] for message in gateway.alert_messages},
        )
        self.assertIn(
            "+17196402230",
            {message["destination"] for message in gateway.alert_messages},
        ...
        self.assertIn(
            "Town of Wiley alert: Town of Wiley developer alert test",
            {message["subject"] for message in gateway.alert_messages},
        )
        self.assertTrue(
            any(
    ...
    def test_developer_test_route_resolves_token_from_managed_secret(self) -> None:
        ...
        self.assertEqual(len(backend._notification_gateway.alert_messages), 1)
        self.assertEqual(
            backend._notification_gateway.alert_messages[0]["destination"],
            "steve.mckitrick@townofwiley.gov",
        )
