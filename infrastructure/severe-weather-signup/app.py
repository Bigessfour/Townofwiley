...
class SubscriptionStore(Protocol):
    def find_existing_subscription(
        self, channel: str, subscriber_key: str
    ) -> dict[str, Any] | None: ...

    def save_subscription(self, item: dict[str, Any]) -> None: ...

    def find_by_token(
        self, token_field: str, token_value: str
    ) -> dict[str, Any] | None: ...
...
class CloudwatchGateway(Protocol):
    def put_metric_data(
        self, namespace: str, metric_name: str, value: float
    ) -> None: ...
...
class NwsClient(Protocol):
    def fetch_active_alerts(
        self, zone_code: str, user_agent: str, api_key: str
    ) -> list[dict[str, Any]]: ...
...
class DynamoSubscriptionStore:
    ...
    def _scan_items(
        ...
        for key, value in equals_filters.items():
            ...
            filter_expression = (
                clause if filter_expression is None else filter_expression & clause
            )
    ...
    def _send(self, channel: str, destination: str, subject: str, message: str) -> None:
        if channel == EMAIL_CHANNEL:
            if not self._sender_email:
                raise RuntimeError(
                    "SENDER_EMAIL must be configured before email subscriptions can be used."
                )
...
class WeatherGovNwsClient:
    def fetch_active_alerts(
        ...
        try:
            ...
            raise RuntimeError(
                f"NWS alert request failed with status {error.code}."
            ) from error
        except URLError as error:
            raise RuntimeError(
                "Unable to reach weather.gov for active alerts."
            ) from error
...
class SevereWeatherBackend:
    ...
    def _handle_log(self, event: dict[str, Any]) -> dict[str, Any]:
        try:
            ...
            return json_response(
                400, {"error": "Invalid JSON body."}, request_event=event
            )
        ...
        if not message:
            return json_response(
                400, {"error": "message is required."}, request_event=event
            )
    ...
    def _handle_developer_test(self, event: dict[str, Any]) -> dict[str, Any]:
        ...
        if not developer_test_token:
            return json_response(
                404, {"error": "Developer test route is disabled."}, request_event=event
            )
        ...
        if provided_token != developer_test_token:
            return json_response(
                403, {"error": "Invalid developer test token."}, request_event=event
            )
        ...
        preferred_language = normalize_alert_language(
            payload.get("preferredLanguage", LANGUAGE_EN)
        )
        zip_code = (
            str(payload.get("zipCode", self._config.allowed_zip_code)).strip()
            or self._config.allowed_zip_code
        )
        event_name = normalize_whitespace(
            str(
                payload.get("event", "Town of Wiley severe weather test alert")
            ).strip(),
        )
        headline = normalize_whitespace(
            str(
                payload.get(
                    "headline",
                    "Developer-only smoke test for the severe weather alert path",
                )
            ).strip(),
        )
        severity = normalize_whitespace(
            str(payload.get("severity", "Moderate")).strip()
        )
        ...
        instruction = normalize_whitespace(
            str(
                payload.get(
                    "instruction",
                    "Confirm delivery on the email address and mobile number provided.",
                )
            ).strip(),
        )
        expires = normalize_whitespace(str(payload.get("expires", "")).strip())
        area_desc = normalize_whitespace(
            str(
                payload.get("areaDesc", "Town of Wiley developer test recipients")
            ).strip()
        )
        alert = {
        ...
        for channel, destination_key in (
            (EMAIL_CHANNEL, "emailDestination"),
            (SMS_CHANNEL, "smsDestination"),
        ):
            ...
            self._notification_gateway.send_alert(
                channel, normalized_destination, subject, message
            )
            destinations_sent.append(
                {"channel": channel, "destination": normalized_destination}
            )
    ...
    def _create_subscription(self, event: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            normalized_destination, subscriber_key = normalize_destination(
                channel, destination
            )
        except ValueError as error:
            log_signup_attempt(
        ...
        existing = self._subscription_store.find_existing_subscription(
            channel, subscriber_key
        )
        request_base_url = build_request_base_url(
            event, self._config.public_api_base_url
        )
        ...
        if existing:
            ...
            confirm_url = build_token_url(
                request_base_url, "/confirm", existing["confirmationToken"]
            )
            unsubscribe_url = build_token_url(
                request_base_url, "/unsubscribe", existing["unsubscribeToken"]
            )
        ...
        unsubscribe_url = build_token_url(
            request_base_url, "/unsubscribe", unsubscribe_token
        )
    ...
    def _confirm_subscription(self, event: dict[str, Any]) -> dict[str, Any]:
        ...
        if not item:
            return html_response(
                404,
                render_status_page(
                    "Link not found", "The confirmation token could not be matched."
                ),
                request_event=event,
            )
        ...
        if item["status"] != ACTIVE_STATUS:
            item = (
                self._subscription_store.update_subscription_status(
                    item["subscriptionId"], ACTIVE_STATUS
                )
                or item
            )
        ...
        return html_response(
            ...
            render_status_page(
                self._localize_message(
                    "Alerts confirmed", item.get("preferredLanguage", LANGUAGE_EN)
                ),
                self._localize_message(
    ...
    def _unsubscribe_subscription(self, event: dict[str, Any]) -> dict[str, Any]:
        ...
        if not token:
            return html_response(
                400,
                render_status_page(
                    "Missing token", "The unsubscribe link is incomplete."
                ),
                request_event=event,
            )
        ...
        if not item:
            return html_response(
                404,
                render_status_page(
                    "Link not found", "The unsubscribe token could not be matched."
                ),
                request_event=event,
            )

        self._subscription_store.update_subscription_status(
            item["subscriptionId"], UNSUBSCRIBED_STATUS
        )
        ...
        return html_response(
            ...
            render_status_page(
                self._localize_message(
                    "Alerts stopped", item.get("preferredLanguage", LANGUAGE_EN)
                ),
                self._localize_message(
    ...
    def _handle_scheduled_event(self) -> dict[str, Any]:
        ...
        subscriptions = self._subscription_store.list_active_subscriptions(
            self._config.allowed_zip_code
        )
        sent_count = 0
        failed_count = 0
    ...
    def _send_confirmation_message(
        ...
        try:
            ...
            raise RuntimeError(
                self._build_confirmation_delivery_error(channel, error)
            ) from error
    ...
    def _build_confirmation_delivery_error(self, channel: str, error: Exception) -> str:
        ...
        if channel == EMAIL_CHANNEL:
            if (
                "sandbox" in normalized_error
                or "email address is not verified" in normalized_error
            ):
                return "Email confirmations are temporarily unavailable. Try SMS text alerts instead or contact Town Hall."
    ...
    def _localize_message(self, message: str, preferred_language: str) -> str:
        ...
        try:
            translated = self._translation_gateway.translate_text(
                masked_message, LANGUAGE_EN, language
            )
        except Exception:
            return message
    ...
    def _publish_metric(self, metric_name: str, value: float) -> None:
        try:
            self._cloudwatch_gateway.put_metric_data(
                self._config.cloudwatch_namespace, metric_name, value
            )
        except Exception as error:
            LOGGER.error(
                json.dumps(
...
def read_config() -> AppConfig:
    return AppConfig(
        ...
        notification_sender_name=(
            os.environ.get(
                "NOTIFICATION_SENDER_NAME", DEFAULT_NOTIFICATION_NAME
            ).strip()
            or DEFAULT_NOTIFICATION_NAME
        ),
        allowed_zip_code=os.environ.get(
            "ALLOWED_ZIP_CODE", DEFAULT_ALLOWED_ZIP_CODE
        ).strip()
        or DEFAULT_ALLOWED_ZIP_CODE,
        alert_zone_code=os.environ.get(
            "ALERT_ZONE_CODE", DEFAULT_ALERT_ZONE_CODE
        ).strip()
        or DEFAULT_ALERT_ZONE_CODE,
        ...
        developer_test_token_secret_name=os.environ.get(
            "DEVELOPER_TEST_TOKEN_SECRET_NAME", ""
        ).strip(),
...
def build_runtime_backend() -> SevereWeatherBackend:
    ...
    return SevereWeatherBackend(
        config=config,
        subscription_store=DynamoSubscriptionStore(
            dynamodb.Table(config.subscriptions_table)
        ),
