...
test.describe('Language Switching', () => {
  test('switches site language and keeps navigation and structure working', async ({ homePage }) => {
    // Navigate home
    await homePage.goto();
    ...
    await expect(homePage.page.locator('html')).toHaveAttribute('lang', /es/i, { timeout: 10000 }).catch(() => {
      // Fallback if lang isn't explicitly set yet
    });

    // Switch back to English
    await homePage.page.locator('#site-language-en').click();
