...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Deploy the Town of Wiley severe weather signup backend."
    )
    ...
    parser.add_argument(
        "--subscriptions-table", default="TownOfWileySevereWeatherSubscriptions"
    )
    parser.add_argument(
        "--deliveries-table", default="TownOfWileySevereWeatherDeliveries"
    )
...
def read_developer_test_token(alert_signup_secrets: dict[str, Any]) -> str:
    ...
    return str(
        secrets.get("weather", {}).get("alertSignup", {}).get("developerTestToken", "")
    ).strip()
...
def ensure_secrets_manager_secret(secret_name: str, secret_value: str) -> str:
    ...
    if not secret_value:
        raise RuntimeError(
            "Developer test token is required before it can be stored in Secrets Manager."
        )
...
def ensure_sns_email_subscription(topic_arn: str, email: str) -> None:
    if not email:
        raise RuntimeError(
            "Alarm notification email is required to subscribe CloudWatch alarm topics."
        )
...
def ensure_cloudwatch_alarm(
    alarm_name: str, namespace: str, metric_name: str, topic_arn: str, description: str
) -> None:
    run_aws(
        [
            "cloudwatch",
...
def ensure_alert_alarms(
    namespace: str, notification_email: str, function_name: str
) -> None:
...
def ensure_role(
    ...
    run_aws(
        [
            ...
            role_name,
            "--policy-name",
            "TownOfWileySevereWeatherPolicy",
            "--policy-document",
            f"file://{policy_path}",
        ],
        expect_json=False,
...
def ensure_schedule(
    rule_name: str, schedule_expression: str, function_name: str, function_arn: str
) -> None:
    rule = run_aws(
        [
            "events",
    ...
    run_aws(
        [
            ...
            rule_name,
            "--targets",
            json.dumps([{"Id": "SevereWeatherBackend", "Arn": function_arn}]),
        ]
    )


def update_function_public_base_url(
    function_name: str, environment: dict[str, str], function_url: str
) -> None:
...
def check_ses_sender(sender_email: str) -> str:
    ...
    try:
        identity = run_aws(
            ["sesv2", "get-email-identity", "--email-identity", sender_email]
        )
    except RuntimeError:
        ...
        if domain:
            try:
                domain_identity = run_aws(
                    ["sesv2", "get-email-identity", "--email-identity", domain]
                )
            except RuntimeError:
...
def main() -> int:
    ...
    developer_test_secret_name = (
        args.developer_test_token_secret_name
        or alert_signup_secrets.get("developerTestTokenSecretName", "")
    ).strip() or "TownOfWileySevereWeatherDeveloperTestToken"
    alarm_notification_email = (
        args.alarm_notification_email
        or alert_signup_secrets.get("alarmNotificationEmail", "")
    ).strip() or "steve.mckitrick@townofwiley.gov"
    cloudwatch_namespace = (
        args.cloudwatch_namespace or alert_signup_secrets.get("cloudwatchNamespace", "")
    ).strip() or "TownOfWiley/SevereWeather"
    developer_test_token = (
        args.developer_test_token.strip()
        or read_developer_test_token(alert_signup_secrets)
    )

    if not nws_user_agent:
        raise RuntimeError(
            "NWS_USER_AGENT is required to deploy the scheduled alert poller."
        )
    ...
    ensure_schedule(
        args.rule_name, args.schedule_expression, args.function_name, function_arn
    )
    ensure_alert_alarms(
        cloudwatch_namespace, alarm_notification_email, args.function_name
    )
