...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--function-url",
        required=True,
        help="TownOfWileySevereWeatherBackend Function URL.",
    )
    parser.add_argument(
        "--token",
        default="",
        help="Developer test token expected by the backend route.",
    )
    parser.add_argument(
        "--email", help="Email destination for the isolated test alert."
    )
    ...
    parser.add_argument(
        ...
        default="en",
        choices=["en", "es"],
        help="Preferred language for the test alert.",
    )
    parser.add_argument("--event", default="Town of Wiley severe weather test alert")
    parser.add_argument(
        "--headline",
        default="Developer-only smoke test for the severe weather alert path",
    )
    parser.add_argument(
        "--instruction",
        default="Confirm delivery on the email address and mobile number provided.",
    )
    ...
    parser.add_argument(
        "--area-desc", default="Town of Wiley developer test recipients"
    )
...
def main() -> int:
    ...
    if not token:
        raise SystemExit(
            "Provide --token or unlock the repo secrets with npm run secrets:unlock."
        )
