...
New in v2.2 (Town of Wiley / Angular):
  - content_inclusion.force_full_embed_globs — always fully embed matching files first
  - priority_content_keywords — content-based priority boost for .ts/.html/.mjs/.py files
  - project_analysis.analyze_angular — @Component/@Injectable/signals/PrimeNG analysis
  - CMS Models & Clerk Editor section in AI-BRIEF.md (regex-parsed from cms-admin sources)
  - --focus <profile> CLI flag with focus_profiles config (extra embeds + branch diff brief)

Usage:
    python scripts/generate-ai-manifest.py [--compact] [--threads N] [--focus PROFILE]
...
Options:
    ...
    --focus PROFILE  Apply a focus profile from config focus_profiles
...
class AIManifestGenerator:
    ...
    def __init__(
        ...
        # v2.2 — files matching these globs always get FULL content embedding
        # (highest embed priority; never preview). never_embed still wins.
        self.force_full_embed_globs: List[str] = list(
            ci.get("force_full_embed_globs", []) or []
        )
        ...
        # v2.2 — content-based priority boosts (keyword → boost points)
        self.priority_content_keywords: Dict[str, int] = (
            self.config.get("priority_content_keywords", {}) or {}
        )
        ...
        # v2.2 — focus profiles (--focus CLI flag)
        self.focus_profiles: Dict[str, Any] = (
            self.config.get("focus_profiles", {}) or {}
        )
        self.focus_name: Optional[str] = None
        self.focus_note: str = ""
        if focus:
            profile = self.focus_profiles.get(focus)
            if profile is None:
                available = ", ".join(sorted(self.focus_profiles)) or "(none defined)"
                print(
                    f"Unknown focus profile '{focus}'. Available profiles: {available}",
                    file=sys.stderr,
                )
                sys.exit(1)
            self.focus_name = focus
            self.focus_note = str(profile.get("brief_note", "")).strip()
            for extra_glob in profile.get("force_full_embed_globs", []) or []:
                if extra_glob not in self.force_full_embed_globs:
                    self.force_full_embed_globs.append(extra_glob)

        self._glob_regex_cache: Dict[str, re.Pattern[str]] = {}

        self.repo_info = self._get_repo_info()
        self._tracked_files = self._load_tracked_file_set()
    ...
    def _glob_to_regex(pattern: str) -> "re.Pattern[str]":
        """Translate a glob pattern (with proper ``**`` semantics) into a regex.

        ``Path.match`` treats ``**`` as a single-segment ``*``, which breaks
        patterns like ``**/cms-admin/**/*.ts``. This translation supports:
          - ``**/`` → zero or more directories
          - ``**``  → anything (including ``/``)
          - ``*``   → anything except ``/``
          - ``?``   → single char except ``/``
        Patterns without a leading ``**/`` are anchored to the repo root.
        """
        out: List[str] = []
        i = 0
        n = len(pattern)
    ...
    def _matches_any_glob(self, rel_str: str, globs: List[str]) -> bool:
        ...
        for g in globs:
            regex = self._glob_regex_cache.get(g)
            if regex is None:
                regex = self._glob_to_regex(g)
                self._glob_regex_cache[g] = regex
    ...
    def _force_full_embed(self, rel_str: str) -> bool:
        """True when the file must always be fully embedded (never previewed).

        never_embed globs win so test/lock files stay out even when a force
        glob would otherwise match.
        """
        if not self.force_full_embed_globs:
            return False
        if self._never_embed(rel_str):
            return False
        return self._matches_any_glob(rel_str, self.force_full_embed_globs)
    ...
    _BOOST_EXTENSIONS = {".ts", ".html", ".mjs", ".py"}
    _BOOST_MAX_SIZE_BYTES = 256 * 1024

    def _content_priority_boost(self, file_path: Path, ext: str, size: int) -> int:
        """Return the max priority boost for files containing configured keywords.

        Only inspects small text files (.ts/.html/.mjs/.py under 256 KB) and is a
        no-op when priority_content_keywords is empty.
        """
        if not self.priority_content_keywords:
            return 0
        if ext not in self._BOOST_EXTENSIONS or size > self._BOOST_MAX_SIZE_BYTES:
            return 0
        try:
            ...
            return 0
        boost = 0
        for keyword, pts in self.priority_content_keywords.items():
            if keyword in content:
                boost = max(boost, int(pts))
        return boost
    ...
    def _get_content(
        self, file_path: Path, rel_str: str, size_kb: float, force_full: bool = False
        ...
        Rules (applied in order):
          ...
          3. force_full OR is_critical OR size_kb <= embed_full_if_smaller_than_kb → full embed
    ...
    def _parse_npm_deps(self) -> List[Dict[str, str]]:
    ...
    def _load_tracked_file_set(self) -> set[str]:
        """Load all git-tracked paths once (forward slashes)."""
        try:
    ...
    def _is_tracked(self, relative_path: Path) -> bool:
        ...
        return rel in self._tracked_files
    ...
    def _attach_content_blocks(self, files: List[Dict[str, Any]]) -> int:
        ...
        Files matching force_full_embed_globs are embedded first (always full
        content), before the remaining files count against max_embedded_files.
        """
        ...
        for file_info in files:
            if self._force_full_embed(file_info["metadata"]["path"]):
                forced.append(file_info)
            else:
                regular.append(file_info)

        embedded_count = 0
        for is_forced, group in ((True, forced), (False, regular)):
            for file_info in group:
                ...
                content_block = self._get_content(
                    file_path, rel_str, size_kb, force_full=is_forced
                )
        ...
        if self.max_threads > 0:
            ...
            def process_file(file_path: Path) -> Optional[Dict[str, Any]]:
                ...
                try:
                    ...
                    boost = self._content_priority_boost(
                        file_path, file_path.suffix.lower(), size
                    )
                    if boost:
                        priority = min(priority + boost, 100)
    ...
    def _analyze_architecture(self) -> Dict[str, Any]:
        ...
        analysis_progress.update()
        analysis_progress.set_description("🅰️ Analyzing Angular / PrimeNG")

        # Angular / PrimeNG analysis if enabled (v2.2)
        angular: Dict[str, Any] = {}
        if self.project_analysis.get("analyze_angular", False):
            angular = self._analyze_angular()
    ...
    # ------------------------------------------------------------------
    # Angular / PrimeNG analysis (v2.2)
    # ------------------------------------------------------------------

    _SIGNAL_USAGE_RE = re.compile(r"\b(?:signal|computed|input|output)\s*\(")
    _PRIMENG_IMPORT_RE = re.compile(r"""from\s+['"]primeng/([\w./-]+)['"]""")

    def _analyze_angular(self) -> Dict[str, Any]:
        """Scan .ts files for Angular components/services, signals usage, and PrimeNG imports."""
        components: List[str] = []
        injectables: List[str] = []
        signals_files = 0
        primeng_modules: Dict[str, int] = {}

        for f in self._files:
            rel = f["metadata"]["path"]
            if not rel.endswith(".ts"):
                continue
            content = self._read_text_file(self.repo_root / rel)
            if not content:
                continue
            ...
            if "@Injectable(" in content:
                injectables.append(rel)
            if "@angular/core" in content and self._SIGNAL_USAGE_RE.search(content):
                signals_files += 1
            for module in self._PRIMENG_IMPORT_RE.findall(content):
                module = module.split("/")[0].lower()
                primeng_modules[module] = primeng_modules.get(module, 0) + 1

        return {
            "components": sorted(components),
            "injectables": sorted(injectables),
            "signals_files": signals_files,
            "primeng_modules": dict(
                sorted(primeng_modules.items(), key=lambda kv: (-kv[1], kv[0]))
            ),
            "counts": {
                "components": len(components),
                "injectables": len(injectables),
                "signals_files": signals_files,
                "primeng_modules": len(primeng_modules),
            },
        }

    # ------------------------------------------------------------------
    # CMS model & clerk task parsing (v2.2)
    # ------------------------------------------------------------------

    _CMS_FLAGGED_FIELDS = {"displayOrder", "groupId", "lineEn", "lineEs", "active"}

    def _parse_cms_models(self) -> List[Dict[str, Any]]:
        """Regex-parse CMS_MODEL_DEFINITIONS from cms-model-inventory.ts."""
        inventory_path = (
            self.repo_root / "src" / "app" / "cms-admin" / "cms-model-inventory.ts"
        )
        ...
        array_match = re.search(
            r"CMS_MODEL_DEFINITIONS[^=]*=\s*\[(.*?)\n\];", text, re.DOTALL
        )
        if not array_match:
            return []
        array_text = array_match.group(1)

        model_matches = list(re.finditer(r"\{\s*model:\s*'([^']+)'", array_text))
        models: List[Dict[str, Any]] = []
        for idx, mm in enumerate(model_matches):
            block_end = (
                model_matches[idx + 1].start()
                if idx + 1 < len(model_matches)
                else len(array_text)
            )
            block = array_text[mm.start() : block_end]

            fields: List[str] = []
            key_fields_match = re.search(r"keyFields:\s*'([^']*)'", block)
            if key_fields_match:
                # Drop parenthetical notes, e.g. "id (town-information, city-clerk)"
                cleaned = re.sub(r"\([^)]*\)", "", key_fields_match.group(1))
                fields = [fld.strip() for fld in cleaned.split(",") if fld.strip()]

            ordered: Optional[Dict[str, str]] = None
            ordered_match = re.search(r"ordered:\s*\{([^}]*)\}", block, re.DOTALL)
            if ordered_match:
                ordered = dict(
                    re.findall(r"(\w+):\s*'([^']+)'", ordered_match.group(1))
                )

            models.append(
                {"name": mm.group(1), "fields": fields, "ordered": ordered}
            )
        return models

    def _parse_clerk_tasks(self) -> Dict[str, List[str]]:
        """Regex-parse CLERK_CMS_TASKS from cms-clerk-tasks.ts → model → [task ids]."""
        tasks_path = self.repo_root / "src" / "app" / "cms-admin" / "cms-clerk-tasks.ts"
        ...
        # Each task object declares id: '...' before model: '...'
        for task_id, model in re.findall(
            r"\bid:\s*'([^']+)'[\s\S]*?\bmodel:\s*'([^']+)'", array_match.group(1)
        ):
            tasks_by_model.setdefault(model, []).append(task_id)
        return tasks_by_model

    def _generate_cms_section(self) -> List[str]:
        """Render the '## CMS Models & Clerk Editor' brief section (or [] on any failure)."""
        try:
            models = self._parse_cms_models()
            if not models:
                return []

            try:
                tasks_by_model = self._parse_clerk_tasks()
            except Exception:
                tasks_by_model = {}

            lines: List[str] = [
                "",
                "## CMS Models & Clerk Editor",
                "Parsed from `src/app/cms-admin/cms-model-inventory.ts` and "
                "`src/app/cms-admin/cms-clerk-tasks.ts`. "
                "**Bold** fields are ordering/grouping/localization keys "
                "(`displayOrder`, `groupId`, `lineEn`, `lineEs`, `active`).",
                "",
            ]
            for model in models:
                ...
                if ordered:
                    ...
                    ordered_desc = f"ordered editor: **yes** ({parts})"
                else:
                    ordered_desc = "ordered editor: no"

                task_ids = tasks_by_model.get(model["name"]) or []
                tasks_desc = (
                    ", ".join(f"`{tid}`" for tid in task_ids) if task_ids else "(none)"
                )

                lines.append(
                    f"- **{model['name']}** — {ordered_desc}; clerk tasks: {tasks_desc}"
                )
                if model["fields"]:
                    rendered_fields = ", ".join(
                        f"**`{fld}`**" if fld in self._CMS_FLAGGED_FIELDS else f"`{fld}`"
                        for fld in model["fields"]
                    )
                    lines.append(f"  - fields: {rendered_fields}")
            return lines
        except Exception:
            return []
    ...
    def _generate_ai_brief(self, manifest: Dict[str, Any]) -> Path:
        ...
        project_name = hints.get("project_name", "Repository")
        project_purpose = hints.get(
            "project_purpose",
            f"{project_name} — {hints.get('project_type', 'application')}.",
        )

        lines: List[str] = [
            f"# {project_name} — AI Briefing",
        ...
        important_context = hints.get("important_context", "").strip()
        if important_context:
            lines += ["**Important Context**:", important_context, ""]

        lines += [
            "## Architecture Patterns",
            f"- **Stack** — {hints.get('project_type', 'application')}",
            f"- **UI** — {hints.get('ui_framework', 'component library')}",
            f"- **Pattern** — {hints.get('architecture_pattern', 'layered application')}",
            f"- **DI** — {hints.get('di_container', 'framework dependency injection')}",
            f"- **Async** — {hints.get('async_init_pattern', 'async initialization patterns')}",
            "",
        ]

        navigation_steps = hints.get("navigation_steps") or []
        if navigation_steps:
            lines.append("## How to Navigate the Active Codebase")
            for i, step in enumerate(navigation_steps, 1):
                lines.append(f"{i}. {step}")
            lines.append("")

        lines.append("## Critical Files (read these first)")
        ...
        # CMS models & clerk editor section (v2.2) — never fails brief generation
        lines.extend(self._generate_cms_section())

        # Angular / PrimeNG section (v2.2)
        angular = arch.get("angular") or {}
        if angular:
            lines += [
                "",
                "## Angular & PrimeNG",
                f"- Components (`@Component`): **{len(angular.get('components', []))}**",
                f"- Injectable services (`@Injectable`): **{len(angular.get('injectables', []))}**",
                f"- Files using signals APIs (`signal`/`computed`/`input`/`output`): "
                f"**{angular.get('signals_files', 0)}**",
            ]
            primeng_modules = angular.get("primeng_modules") or {}
            if primeng_modules:
                lines.append(
                    f"- PrimeNG modules imported: **{len(primeng_modules)}** — top usage:"
                )
                for module, count in list(primeng_modules.items())[:12]:
                    lines.append(f"  - `primeng/{module}` ({count} imports)")
        ...
        npm_deps = manifest.get("npm_dependencies") or []
        nuget_deps = manifest.get("nuget_dependencies") or []
        if npm_deps:
            lines += ["", "## Key npm Dependencies"]
            for d in npm_deps[:25]:
                lines.append(f"- `{d['package']}` v{d['version']}")
        if nuget_deps:
            lines += ["", "## Key NuGet Dependencies"]
            for d in nuget_deps[:25]:
                lines.append(f"- `{d['package']}` v{d['version']}")

        # Recent branch changes section (v2.2) — only when a focus profile is active
        if self.focus_name:
            change_lines: List[str] = []
            try:
                diff_out = self._git("diff", "--name-status", "origin/main...HEAD")
                if diff_out:
                    change_lines.append("# git diff --name-status origin/main...HEAD")
                    change_lines.extend(diff_out.splitlines())
                status_out = self._git("status", "--porcelain")
                if status_out:
                    change_lines.append("# git status --porcelain")
                    change_lines.extend(status_out.splitlines())
            except Exception:
                pass
            if not change_lines:
                change_lines = ["(no changes detected vs origin/main)"]

            lines += ["", "## Recent Branch Changes", "```"]
            lines.extend(change_lines[:100])
            if len(change_lines) > 100:
                lines.append(f"... ({len(change_lines) - 100} more lines truncated)")
            lines.append("```")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Apply a focus profile from config focus_profiles (extra full embeds + branch diff in brief)",
    )
    ...
    try:
        generator = AIManifestGenerator(
            repo_root, max_threads=args.threads, focus=args.focus
        )

        if generator.focus_name:
            print(f"Focus profile active: {generator.focus_name}")
