...
def ensure_bucket(bucket_name: str, region: str) -> str:
    try:
        ...
        run_aws(
            ["s3api", "wait", "bucket-exists", "--bucket", bucket_name],
            expect_json=False,
            region=region,
        )
...
def ensure_lambda_s3_permission(
    function_name: str, bucket_arn: str, account_id: str, region: str
) -> None:
    try:
        run_aws(
            [
...
def ensure_bucket_notification(
    bucket_name: str, function_arn: str, prefix: str, region: str
) -> None:
...
def ensure_receipt_rule_set(rule_set_name: str, region: str) -> None:
    try:
        run_aws(
            ["ses", "describe-receipt-rule-set", "--rule-set-name", rule_set_name],
            region=region,
        )
    except RuntimeError:
        run_aws(
            ["ses", "create-receipt-rule-set", "--rule-set-name", rule_set_name],
            region=region,
        )
...
def ensure_active_receipt_rule_set(rule_set_name: str, region: str) -> None:
    run_aws(
        ["ses", "set-active-receipt-rule-set", "--rule-set-name", rule_set_name],
        expect_json=False,
        region=region,
    )
...
def require_value(value: str, label: str) -> str:
    ...
    raise SystemExit(
        f"Missing required value for {label}. Provide it via the CLI flag or secrets/local/user-secrets.json."
    )
...
def main() -> int:
    ...
    ingress_region = resolve_value(
        args.ingress_region, mail_defaults.get("ingressRegion"), "us-east-1"
    )
    account_id = get_account_id(str(aws_secrets.get("accountId", "")), ingress_region)
    alias_domain = resolve_value(
        args.alias_domain, mail_defaults.get("aliasDomain"), "townofwiley.gov"
    )
    ...
    receipt_prefix = resolve_value(
        args.receipt_prefix, mail_defaults.get("receiptPrefix"), "incoming/"
    )
