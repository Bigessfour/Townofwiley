...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Deploy the Town of Wiley developer site monitor."
    )
...
def ensure_schedule(
    ...
    run_aws(
        [
            ...
            rule_name,
            "--targets",
            json.dumps([{"Id": "TownSiteMonitor", "Arn": function_arn}]),
        ]
    )
...
def main() -> int:
    ...
    appsync_endpoint = args.appsync_endpoint.strip() or amplify_config.get(
        "endpoint", ""
    )
    ...
    if not appsync_endpoint or not appsync_api_key:
        raise RuntimeError(
            "The AppSync CMS endpoint and API key are required before deployment can continue."
        )

    site_url = args.site_url.strip() or "https://townofwiley.gov"
    admin_url = args.admin_url.strip() or f'{site_url.rstrip("/")}/admin'
    state_table_region = (
        args.state_table_region.strip() or os.environ.get("AWS_REGION", "").strip()
    )
