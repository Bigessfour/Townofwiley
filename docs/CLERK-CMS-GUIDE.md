...
| **imageUrl**                       | Optional photo **or** public webpage for the bulletin card.               | Must start with **`https://`**. A direct image (`.jpg`, `.png`, `.webp`) shows on `/notices` and `/news` cards. An ArcGIS StoryMap or other webpage is **not** shown as a picture — the card opens that page in a new tab (with an **Open interactive Story Map** / **Open link** button). Do not paste a StoryMap into this field expecting an embedded map. Leave blank for routine text-only notices. The homepage timeline does not show photos; a StoryMap notice there still opens the story. |
