# Town of Wiley — Operations Runbook

← [Back to README](../README.md)

Maintainer-facing deployment, infrastructure, secrets, and service runbooks. For a project overview and quickstart, see the [README](../README.md).
...
- Keep maintainer-facing docs and runbooks tracked in the repo under `docs/`, `CLERK-CMS-GUIDE.md`, `bot-training/`, and related operational paths.
...
Detailed policy: [docs/git-workflow.md](git-workflow.md)

Branch protection and required CI gate: [docs/github-branch-protection.md](github-branch-protection.md)

## Runtime Baseline

- **Node.js `24.x` LTS only** for this app (Amplify, GitHub Actions, and `package.json` / `ensure-node-version` agree). **`engines.node` is `>=24.15.0 <25.0.0`** (with `.npmrc` `engine-strict=true`). Repo files pin **`24.16.0`** (`.nvmrc`, `.node-version`, **`volta`**, **`mise.toml`**, **asdf** `.tool-versions`) — see **[`docs/NODE_VERSION.md`](NODE_VERSION.md)** for why the pin is an exact patch (not “whatever LTS says today”). **Do not use Node 22, 23, 25+, or odd majors** — Node 25+ has caused toolchain and native dependency issues with this stack.
- Use `nvm install && nvm use` (reads `.nvmrc`), **`mise install`**, **Volta**, **asdf**, Homebrew **`node@24`**, Windows **`.\scripts\setup-repo-node.ps1`**, or **Docker `node:24-slim`** (see [`docs/NODE_VERSION.md`](NODE_VERSION.md) § Docker) so your shell `node` matches CI.
...
- **Deploy steps:**
  - **Automatic (normal):** merge to `main` with deployable app changes → Site CI builds and uploads artifact → `deploy-production` syncs to S3 + CloudFront invalidation. See [`docs/github-actions-production-deploy.md`](github-actions-production-deploy.md).
  - **Manual (break-glass):**
  ...
  The `scripts/ansible-deploy.sh` wrapper + root `ansible.cfg` hide sourcing and config details. See [`ansible/README.md`](../ansible/README.md) and `package.json` scripts.
...
| Hosting build + CSP origins + SPA rules                   | [`customHttp.yml`](../customHttp.yml) (CSP SSOT for dev parity), [`docs/third-party-csp-registry.md`](third-party-csp-registry.md), `scripts/generate-static-route-entrypoints.mjs`                |
| Operator runbook + deploy order                           | [`docs/AWS_INFRASTRUCTURE_SOT.md`](AWS_INFRASTRUCTURE_SOT.md)                                                                                                                                   |
| Full product / AP tracker                                 | [`docs/post-development-inventory.md`](post-development-inventory.md)                                                                                                                           |
...
- Costs now free-tier safe. See [`docs/aws-cost-optimization-runbook.md`](aws-cost-optimization-runbook.md) for details.
...
Verify live CMS after deploy: `npm run verify:runtime-config-cms`. API key rotation: [`docs/appsync-api-key-rotation-runbook.md`](appsync-api-key-rotation-runbook.md).
...
- [`docs/README.md`](README.md) — documentation index and current status
...
- Follow the operator steps in [docs/town-email-alias-forwarding-runbook.md](town-email-alias-forwarding-runbook.md).
...
This section tracks post-build remediation. **Source of truth:** [`docs/post-development-inventory.md`](post-development-inventory.md) and [`docs/README.md`](README.md). AP-03 (Paystar placeholder) merged 2026-05-22.
