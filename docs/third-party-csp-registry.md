...
| `style-src-attr 'unsafe-inline'`             | Angular emulated encapsulation and PrimeNG set `style=""` on components                                                             | `style-src 'self'` alone blocks `<style>` tags from bundles; attribute styles need `style-src-attr`.                                                                                                            |
| `style-src 'unsafe-inline'` (dev serve only) | Vite `ng serve` injects component `<style>` blocks at runtime                                                                       | `npm run sync:angular-serve-csp` adds this to `angular.json` serve headers only; production `customHttp.yml` stays `style-src 'self'`.                                                                          |
