...
Gen 2 staff pool **`us-east-2_pkewJMUJF`** (client `2av73ehrkera414otok5i67dk3` "townofwiley-staff-web" as of 2026-06) should use **Amazon SES** (not `COGNITO_DEFAULT`) for reliable reset codes:

The client supports **both** direct username/password (the custom form at `/admin/login`) **and** OAuth Authorization Code redirect to the hosted UI (`https://townofwiley-staff.auth.us-east-2.amazoncognito.com/login` with callbacks registered exactly for `https://townofwiley.gov/admin/login` and www.).

See `src/app/auth/admin-login.component.ts` + `.html` (has "Sign in via Cognito hosted page" button) and `src/app/amplify-config.ts` (oauth config + fallback).
