...
| **Utility payments (Paystar)** | Browser uses hosted portal `href` only via `resolveQuickPayHref()` (`src/app/payments/paystar-quick-pay.ts`). CTAs disabled when `portalUrl` empty (AP-03, PR #30). Hosted Paystar portal only; no API/proxy path. | `/services` + `/pay-bill`; E2E `enablePaystarHostedWithoutPortal` in `payments.spec.ts`                          |
