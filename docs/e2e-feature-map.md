...
| **Utility payments (Paystar)** | Hosted portal URL and/or town proxy (`infrastructure/paystar-proxy/`). **Embedded Session API** scaffold: POST with `sessionType` (e.g. `payment` → `/integrations/embedded/initiate`) per [Paystar Embedded docs](https://docs.paystar.io/api/embedded/) — see `docs/post-development-inventory.md` Phase 6. `PaystarConnectionService` not yet wired to UI. | `/services` billing help + portal button; `/payments` full form; E2E mocks `/e2e-mock-paystar`                   |
