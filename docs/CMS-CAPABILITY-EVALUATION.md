...
2. **P1 — Production `OfficialContact` data is missing its stable IDs.** A read-only scan shows exactly one row with a random UUID id (`94d5fe8f-…`); the site looks up cards by `town-information` and `city-clerk`. Footer/permits/contact cards therefore use bundled fallbacks, and a clerk editing the one existing row may not see their change where they expect. IT should restore the two stable-ID rows.
...
- **SiteCopy is omitted from persisted snapshots** — it only hydrates from the live extended query. After SiteCopy is deployed, a clerk's label edit may take an extra live fetch to appear while other content shows instantly. Nothing in clerk-facing copy mentions this; worth a one-line note in the SiteCopy task help once the model is live (P2).
