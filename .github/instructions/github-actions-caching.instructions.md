...
| Ollama model weights | `./.github/actions/cache-ollama-model` | Ollama PR review + CI diagnosis before `ollama pull` |
...
3. **Ollama:** default model on hosted runners is `llama3.2:3b`; override via repo variables `OLLAMA_CI_MODEL` / `OLLAMA_PR_REVIEW_MODEL`.
...
On CI gate failure, `ollama-ci-diagnosis` uploads `outputs/` including `04-ci-improvements.json`. Maintainers can run:
