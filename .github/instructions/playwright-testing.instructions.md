...
- **Agentic / MCP**: After Playwright upgrades, run **`npm run e2e:init-agents`** to refresh `playwright-test-*` agent templates. Enable MCP in the IDE so [`.vscode/mcp.json`](../../.vscode/mcp.json) and [`.cursor/mcp.json`](../../.cursor/mcp.json) expose **`playwright-test`** (run tests / `test_run`) and **`microsoft/playwright-mcp`** (explore pages only). **`@playwright/mcp` is 0.x** (e.g. `^0.0.75`); **`@playwright/test` is 1.x** — never pin `@playwright/mcp@1.59.x` (npm `ETARGET`). Run **`npm run verify:playwright-mcp`** after MCP config changes. For pre-push checks without MCP, use VS Code tasks **`e2e: smoke (CI parity)`** or **`e2e: pre-push (preflight + smoke)`** in [`.vscode/tasks.json`](../../.vscode/tasks.json). See [`e2e/README.md`](../../e2e/README.md) and [`.github/agents/README.md`](../agents/README.md).
