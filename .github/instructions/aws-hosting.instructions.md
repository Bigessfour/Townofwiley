...
- For frontend deploys: build → tiered-cache s3 sync (see README "Deploy steps" and manifest hosting.deployCommand for --cache-control passes) → `aws cloudfront create-invalidation --distribution-id E1NZ3XCY5CYR1J --paths "/*"`. Current CF uses managed CachingOptimized + custom Response Headers Policy (CSP/security) + logging.
